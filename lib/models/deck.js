...
module.exports = class Deck {
  ...
  static async getCardCount(deckId) {
    const { rows } = await pool.query(
      `select decks_cards.sk_id, mtg_cards.name, decks_cards.sideboard, count(decks_cards.sk_id) as quantity
    from decks_cards
    JOIN mtg_cards ON decks_cards.sk_id = mtg_cards.sk_id
    WHERE decks_cards.deck_id = $1
    group by decks_cards.sk_id, mtg_cards.name, decks_cards.sideboard
